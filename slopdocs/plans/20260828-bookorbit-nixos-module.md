...
- Require a fully-qualified public `domain`; the module owns its HTTPS Caddy vhost with gzip/zstd compression and the repository reaction log format. Keep a 127.0.0.1 container port mapping by default so the app’s HTTP listener cannot bypass the proxy.
